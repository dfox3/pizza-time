...
shutil.rmtree('src/dfizza.egg-info', ignore_errors=True);
for f in glob.glob('src/dfizza/*.so'): os.remove(f);
for f in glob.glob('src/dfizza/*.c'): os.remove(f);
"""
...
def precommit(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "dev", "--group", "lint", "--group", "ty", external=True,
    )
...
def mypy(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "dev", "--group", "mypy", external=True,
    )
...
def ty(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "ty", external=True, env={"UV_PROJECT_ENVIRONMENT": session.virtualenv.location},
    )
...
def tests(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "dev", external=True, env={"UV_PROJECT_ENVIRONMENT": session.virtualenv.location},
    )
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"DFIZZA_COMPILE_MYPYC": "1"})
...
def coverage(session: nox.Session) -> None:
    ...
    session.install(
        "pytest", "coverage[toml]", "pytest-cov", "pytest-mock", "pytest-xdist", "pytest-randomly",
    )
    session.install("-e", ".")
...
def typeguard_tests(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "dev", "--group", "typeguard", external=True,
    )
...
def xdoctest(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "dev", "--group", "xdoctest", external=True,
    )
...
def docs_build(session: nox.Session) -> None:
    ...
    session.run(
        "uv", "sync", "--group", "docs", external=True, env={"UV_PROJECT_ENVIRONMENT": session.virtualenv.location},
    )
